...
export class NextJsSite extends pulumi.ComponentResource {
	...
	private createOrigins() {
		...
		const defaultFunctionUrl = this.createFunctionOrigin(
			"default",
			defaultOrigin as OpenNextFunctionOrigin,
		);
		const imageFunctionUrl = this.createFunctionOrigin(
			"image-optimizer",
		...
		return [
			...
			{
				originId: "default",
				domainName: defaultFunctionUrl.functionUrl.apply(
	...
	private createFunctionOrigin(key: string, origin: OpenNextFunctionOrigin) {
		...
		const role = new aws.iam.Role(
			`${this.name}-${key}-origin-lambda-role`,
			{
		...
		return new aws.lambda.FunctionUrl(
			`${this.name}-${key}-origin-lambda-url`,
			{
				functionName: fn.arn,
	...
	grantPermissions(role: aws.iam.Role, key: string) {
		...
		new aws.iam.RolePolicyAttachment(
			`${this.name}-${key}-lambda-basic-execution-role-polict-attachment`,
			{
				policyArn:
		...
		new aws.iam.RolePolicyAttachment(
			`${this.name}-${key}-bucket-read-write-role-policy-attachment`,
			{
		...
		new aws.iam.RolePolicyAttachment(
			`${this.name}-${key}-table-read-write-data-role-policy-attachment`,
			{
		...
		new aws.iam.RolePolicyAttachment(
			`${this.name}-${key}-queue-send-message-role-policy-attachment`,
			{
